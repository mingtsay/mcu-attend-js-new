/* MCU Attend, Version 1.0.1 */
/* This script release under LGPL License */

var mcu_attend_version = "1.0.1";
...
//        window.location.href = "http:" + window.location.href.substring(window.location.protocol.length);
    ...
    loadJQ(function () {
        loadJQ(function () {
            ...
            if($(target_document).find("form #ctl00_ContentPlaceHolder1_PnlStno table table td:has(input)").length == 0) {
                window.alert("唱名系統載入失敗，請確認您所開啟的頁面是否為銘傳的點名考勤系統！");
                return;
            }

            $(target_document).find("form #ctl00_ContentPlaceHolder1_PnlStno table table td:has(input)").mouseover(function() {
                $(this).addClass("hover");
            }).mouseout(function() {
            ...
            $(target_document).find("form #ctl00_ContentPlaceHolder1_PnlStno table td input").focus(function() {
                ...
                global_now_index = parseInt($(this).parents("td.mcu_attend_students").attr("mcu_attend_index"), 10);
            }).blur(function() {
                $(this).parents("td.mcu_attend_students").removeClass("focus");
                if (global_now_index == parseInt($(this).parents("td.mcu_attend_students").attr("mcu_attend_index"), 10)) {
                    global_now_index = -1;
                }
            }).change(function() {
                if (this.checked) {
                    if (target_document.getElementById("mcu_attend_player_checkbox").checked) {
                        playSound($(this).parents("td.mcu_attend_students").attr("mcu_attend_sid"), $(this).parents("td.mcu_attend_students").attr("mcu_attend_name"));
                        setTimeout(function() {
